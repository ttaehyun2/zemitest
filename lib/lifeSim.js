  ...
  },
  {
    ...
    scenes: [
      ...
      { id: "t2", text: "친구들이 위험한 짓을 하자고 한다. 안 가면 겁쟁이 소리를 듣는다.",
        choices: [
          { t: "따라간다", e: { social: 8, health: -8, happy: 5 }, r: "재밌었다. 크게 다칠 뻔했다.", f: "risky", next: "t2a" },
          { t: "거절한다", e: { social: -6, skill: 4, happy: -3 }, r: "한동안 어색했다.", next: "t2b" },
          { t: "말리다가 같이 욕먹는다", e: { social: -4, fame: 3, happy: -2 }, r: "아무도 고마워하지 않았다.", f: "brave", next: "t2b" },
          { t: "가는 척하고 몰래 빠진다", e: { social: 2, happy: 2 }, r: "요령만 늘었다.", f: "sly", next: "t2a" },
        ]},

      { id: "t2a", branch: true, text: "그날 일이 학교에 알려졌다. 누가 주동자였냐고 묻는다.",
        choices: [
          { t: "내가 그랬다고 한다", e: { fame: 8, social: 10, happy: -6 }, r: "혼났지만 친구들이 기억한다.", f: "standup" },
          { t: "아무도 말 안 한다", e: { social: 6, happy: -4 }, r: "다 같이 벌을 받았다." },
          { t: "친구 이름을 댄다", e: { social: -16, happy: -8 }, r: "그 뒤로 아무도 나를 부르지 않았다.", f: "jerk" },
          { t: "모른다고 잡아뗀다", e: { skill: 4, happy: -2 }, r: "넘어갔다. 찝찝함은 남았다.", f: "sly" },
        ]},

      { id: "t2b", branch: true, text: "혼자 있는 시간이 늘었다. 그 시간을 뭘로 채울지 정해야 한다.",
        choices: [
          { t: "몰두할 걸 하나 찾는다", e: { skill: 14, happy: 8 }, r: "그때 시작한 게 오래갔다.", f: "hobby" },
          { t: "책과 영상으로 시간을 보낸다", e: { skill: 8, happy: 5, health: -4 }, r: "머릿속이 넓어졌다." },
          { t: "새로운 무리를 찾는다", e: { social: 12, happy: 6 }, r: "나랑 더 맞는 사람들이 있었다." },
          { t: "아무것도 안 한다", e: { happy: -6, health: 4 }, r: "그 시절이 통째로 비어 있다." },
        ]},
  ...
  },
  {
    ...
    scenes: [
      { id: "w1", text: "진로를 정해야 한다. 각자 대가가 다르다.",
        choices: [
          { t: "안정적인 길을 간다", e: { money: 15, health: 5, happy: -4, fame: 2 }, r: "부모님이 웃었다. 나는 조금 답답했다.", f: "safe", next: "w1a" },
          { t: "하고 싶은 일에 뛰어든다", e: { happy: 14, money: -10, skill: 10, fame: 5 }, r: "통장은 비었지만 눈은 살아 있었다.", f: "passion", next: "w1b" },
          { t: "돈이 되는 쪽으로 간다", e: { money: 22, happy: -8, health: -6 }, r: "빨리 벌었다.", f: "greedy", next: "w1c" },
          { t: "일단 아무거나 한다", e: { money: 6, skill: 4, happy: -2 }, r: "방향은 나중에 찾기로 했다.", next: "w1d" },
        ]},

      { id: "w1a", branch: true, text: "안정된 자리에 앉은 지 3년. 매일이 똑같다. 후배가 하나 들어왔다.",
        choices: [
          { t: "여기서 최고가 되기로 한다", e: { skill: 14, money: 10, fame: 6 }, r: "안정 속에서도 올라갈 길은 있었다." },
          { t: "몰래 다른 걸 준비한다", e: { skill: 12, happy: 8, health: -6 }, r: "퇴근 후의 시간이 진짜 내 시간이었다.", f: "sidehustle" },
          { t: "후배를 잘 키운다", e: { social: 14, happy: 8 }, r: "그 후배는 나중에 나를 찾아왔다.", f: "mentor" },
          { t: "이대로도 괜찮다고 받아들인다", e: { happy: 10, health: 8, skill: -4 }, r: "욕심을 내려놓자 편해졌다." },
        ]},

      { id: "w1b", branch: true, text: "하고 싶은 일을 시작한 지 2년. 수입이 거의 없다. 주변에서 언제까지 할 거냐고 묻는다.",
        choices: [
          { t: "1년만 더 버틴다", e: { skill: 16, money: -12, happy: 6, health: -8 }, r: "그 1년이 갈림길이었다.", f: "persist" },
          { t: "알바를 병행한다", e: { money: 10, skill: 6, health: -10 }, r: "잠을 줄여 둘 다 했다.", f: "worker" },
          { t: "접고 안정적인 길로 간다", e: { money: 16, happy: -12 }, r: "현실을 택했다. 가끔 그 시절이 떠오른다.", f: "gaveup" },
          { t: "방식을 완전히 바꿔본다", e: { skill: 10, fame: 10, money: 4 }, r: "고집을 버리자 길이 보였다." },
        ]},

      { id: "w1c", branch: true, text: "통장에 숫자가 쌓인다. 그런데 요즘 무슨 요일인지 자주 헷갈린다.",
        choices: [
          { t: "속도를 유지한다", e: { money: 20, health: -14, happy: -8 }, r: "더 벌었다. 더 비었다.", f: "burnout" },
          { t: "쉬는 날을 만든다", e: { health: 12, happy: 12, money: -6 }, r: "쉬어보니 알겠더라." },
          { t: "번 돈으로 다른 걸 시작한다", e: { money: -12, skill: 12, fame: 8 }, r: "돈이 나를 자유롭게 했다.", f: "founder" },
          { t: "다 그만두고 여행을 떠난다", e: { money: -18, happy: 20, health: 10 }, r: "미쳤다는 소리를 들었다. 웃겼다.", f: "reborn" },
        ]},

      { id: "w1d", branch: true, text: "몇 년째 정하지 못했다. 동창회에 나갔더니 다들 자리를 잡았다.",
        choices: [
          { t: "조급해져서 아무거나 붙잡는다", e: { money: 10, happy: -8, skill: 4 }, r: "그래도 시작은 했다." },
          { t: "이제라도 제대로 정한다", e: { skill: 14, happy: 8, money: -6 }, r: "늦게 시작한 만큼 진심이었다.", f: "latestart" },
          { t: "남과 비교하지 않기로 한다", e: { happy: 14, health: 6 }, r: "내 속도라는 게 있었다." },
          { t: "동창회를 나오지 않는다", e: { social: -10, happy: 4 }, r: "비교할 대상이 없으니 편했다." },
        ]},
  ...
  },
  {
    ...
    scenes: [
      ...
      { id: "h2", text: "가정을 꾸릴지 결정할 시점이다.",
        choices: [
          { t: "가정을 꾸린다", e: { happy: 16, social: 12, money: -14, health: -4 }, r: "돈은 줄었고 이유는 늘었다.", f: "family", next: "h2a" },
          { t: "혼자 살기로 한다", e: { money: 12, happy: 4, social: -8 }, r: "자유로웠다. 가끔 조용했다.", f: "solo", next: "h2b" },
          { t: "아직 미룬다", e: { money: 6, skill: 6, happy: -2 }, r: "언젠가라는 말을 오래 썼다.", next: "h2b" },
          { t: "일에 모든 걸 쏟는다", e: { money: 18, skill: 12, happy: -12, health: -8 }, r: "성공했다는 말은 들었다.", f: "chosework", next: "h2c" },
        ]},

      { id: "h2a", branch: true, text: "가족이 생기고 나니 결정 하나하나가 무거워졌다. 좋은 기회가 왔는데 위험이 크다.",
        choices: [
          { t: "가족을 위해 안전하게 간다", e: { money: 8, happy: 8, health: 6 }, r: "지킬 게 생기니 겁이 늘었다. 그게 나쁘지만은 않았다." },
          { t: "가족을 위해서라도 도전한다", e: { money: 16, health: -10, happy: -4 }, r: "잘 되면 영웅, 아니면 죄인이었다.", f: "gambler" },
          { t: "가족과 상의해서 정한다", e: { social: 12, happy: 12, money: 6 }, r: "혼자 지고 있던 걸 나눴다." },
          { t: "일을 줄이고 시간을 준다", e: { happy: 16, money: -10, social: 8 }, r: "돈보다 남는 게 있었다." },
        ]},

      { id: "h2b", branch: true, text: "명절이나 모임에서 매번 같은 질문을 받는다. 슬슬 지친다.",
        choices: [
          { t: "당당하게 내 방식이라고 말한다", e: { happy: 12, fame: 4, social: -4 }, r: "몇 번 말하니 아무도 안 물었다.", f: "confident" },
          { t: "적당히 웃어넘긴다", e: { social: 5, happy: -4 }, r: "매번 조금씩 닳았다." },
          { t: "그 자리를 아예 안 나간다", e: { social: -12, happy: 8 }, r: "편해졌다. 멀어지기도 했다." },
          { t: "비슷한 사람들을 찾는다", e: { social: 14, happy: 12 }, r: "혼자가 아니었다는 걸 알았다." },
        ]},

      { id: "h2c", branch: true, text: "일에 다 쏟은 지 몇 년. 성과는 났는데 어느 날 아침 몸이 안 움직인다.",
        choices: [
          { t: "병원에 가고 속도를 줄인다", e: { health: 16, money: -8, happy: 8 }, r: "몸이 먼저 신호를 보냈다." },
          { t: "약 먹고 계속한다", e: { money: 16, health: -20, fame: 8 }, r: "버텼다. 대가는 뒤에 왔다.", f: "ignoredbody" },
          { t: "잠깐 멈추고 생각한다", e: { happy: 10, skill: 6, money: -4 }, r: "멈춰야 보이는 게 있었다." },
          { t: "일을 넘기고 물러난다", e: { money: -10, happy: 14, health: 12, fame: -6 }, r: "내려놓는 것도 용기였다." },
        ]},
  ...
  },
  {
    ...
    scenes: [
      ...
      { id: "m3", text: "모든 걸 정리하고 완전히 다른 삶으로 갈 기회가 있다.",
        choices: [
          { t: "전부 정리하고 떠난다", e: { happy: 22, money: -22, health: 12, fame: -8 }, r: "미친 짓이라고들 했다. 나는 웃었다.", f: "reborn", next: "m3a" },
          { t: "남는다", e: { money: 10, happy: -4 }, r: "안정을 택했다.", next: "m3b" },
          { t: "절반만 바꾼다", e: { happy: 10, money: -6, health: 5 }, r: "다 버리지 않고도 달라질 수 있었다.", next: "m3a" },
          { t: "때를 기다린다", e: { money: 6, happy: -6 }, r: "그 때는 오지 않았다.", next: "m3b" },
        ]},

      { id: "m3a", branch: true, text: "새로 시작한 곳에서 아무도 나를 모른다. 다시 바닥부터다.",
        choices: [
          { t: "처음부터 배운다", e: { skill: 16, happy: 12, money: -8 }, r: "나이는 숫자였다.", f: "humble" },
          { t: "예전 경력을 살린다", e: { money: 14, fame: 8, skill: 5 }, r: "쌓아둔 게 어디 가지 않았다." },
          { t: "여기서는 그냥 쉰다", e: { health: 16, happy: 16, money: -10 }, r: "쉬려고 온 거였다." },
          { t: "비슷한 사람들을 모은다", e: { social: 18, fame: 8, happy: 10 }, r: "혼자 떠났는데 여럿이 됐다.", f: "mentor" },
        ]},

      { id: "m3b", branch: true, text: "남기로 했다. 그런데 후배들이 하나둘 떠나고 자리가 비어간다.",
        choices: [
          { t: "끝까지 자리를 지킨다", e: { money: 12, fame: 6, happy: -4 }, r: "누군가는 남아야 했다.", f: "loyal" },
          { t: "떠나는 사람들을 응원한다", e: { social: 14, happy: 10 }, r: "붙잡지 않는 것도 방법이었다.", f: "mentor" },
          { t: "나도 준비를 시작한다", e: { skill: 12, money: -4, happy: 8 }, r: "늦었지만 시작은 했다." },
          { t: "변화를 직접 만든다", e: { fame: 14, skill: 10, health: -8 }, r: "떠나는 대신 바꾸기로 했다.", f: "bigshot" },
        ]},
